...
MD//WORKS PROVENANCEでは、Editorで入力した通常の改行はPreviewでもそのまま改行として表示されます。
...
MD//WORKS PROVENANCEでは、Markdown文書内に基本的なLaTeX数式を記述し、プレビューで数式として表示できます。

インライン数式は、数式を1つの `$` で囲みます。
...
［Format］メニューまたはツールバーの **Inline Math** / **Display Math** を使って、選択した文字列を数式記法で囲むこともできます。

* **Inline Math** — `$...$` を使用して文中に数式を配置します。
* **Display Math** — `$$...$$` を使用して独立した数式として表示します。

文字として `$` を表示したい場合は、`\$` と記述します。

Inline CodeやCode Block内の `$...$` は数式として解釈されず、コードのまま表示されます。

数式はプレビュー、印刷プレビュー、PDF、および対応する保存レポートで表示されます。Markdown本文にはLaTeX形式の数式ソースがそのまま保持されます。

> **Note:**
> MD//WORKS PROVENANCEの数式機能は、基本的なLaTeX数式を表示するための機能です。完全なLaTeX環境ではありません。式番号、`\label` / `\ref`、独自マクロ、TikZなどの高度なLaTeX機能はサポート対象外です。正しく解釈できない数式は、文書やプレビューを壊さず、可能な範囲で元の記述として表示されます。

#### 数式と執筆記録

数式も通常のMarkdown本文の一部として保存され、文書の整合性検証の対象になります。

キーボードから直接入力した数式は通常の直接入力として記録され、外部から貼り付けた数式は通常の貼り付け来歴に従って記録されます。
...
* Markdown脚注の自動リンク表示
* 式番号、相互参照、独自マクロ、TikZ等の高度なLaTeX機能
* Mermaid図のレンダリング
...
### 10-10. 高度なLaTeX数式・Mermaid・脚注がプレビュー（Preview）で表示されない
...
等の専用レンダリングは保証していません。基本的なLaTeX数式については「3-3. 基本的なLaTeX数式」を参照してください。
